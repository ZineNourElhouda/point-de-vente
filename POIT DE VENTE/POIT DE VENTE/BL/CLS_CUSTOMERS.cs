...
namespace POIT_DE_VENTE.BL
{
    class CLS_CUSTOMERS
    {
        public void ADD_CUSTOMER(string First_Name, string Last_Name, string Tel, string Email, byte[] Picture, string Criterion)
        {
            ...
            param[3] = new SqlParameter("@Criterion", SqlDbType.NVarChar, 50);
            param[3].Value = Criterion;

            DAL.ExecuteCommed("ADD_CUSTOMER", param);
            DAL.Close();
        }

     
    
         void EDIT_CUSTOMER(string First_Name, string Last_Name, string Tel, string Email, byte[] Picture, string Criterion, int ID)
         {
             DAL.DataAccessLayer DAL = new DAL.DataAccessLayer();
             DAL.Open();
             ...
             param[4] = new SqlParameter("@Picture", SqlDbType.Image);
             param[4].Value = Picture;
             param[5] = new SqlParameter("@Criterion", SqlDbType.NVarChar, 50);
             param[5].Value = Criterion;
             param[6] = new SqlParameter("@ID", SqlDbType.Int);
             param[6].Value = ID;
             DAL.ExecuteCommed("EDIT_CUSTOMER", param);
             DAL.Close();
        ...
        public void DELETE_CUSTOMER (int ID)
        {
            ...
            param[0].Value = ID;
            DAL.ExecuteCommed("DELETE_CUSTOMER", param);
            DAL.Close();
